...
async def cmd_create_pr(args: argparse.Namespace) -> int:
    ...
    run_url = (
        ...
        f"{os.environ.get('GITHUB_REPOSITORY', '').strip()}/actions/runs/{run_id}"
    ...
    try:
        title = args.title or (prompt if prompt else f"Bot: {args.job_id}")
        body = args.body or "\n".join(
            line
            for line in [
                "## Summary",
                "Automated changes generated by Agent Runner.",
                "",
                "## Context",
                f"- **Job ID**: {args.job_id}",
                f"- **Upstream repo**: {args.upstream_repo}",
                f"- **Fork repo**: {args.fork_repo}",
                f"- **Branch**: {args.branch}",
                f"- **Prompt**: {prompt if prompt else '(not provided)'}",
                f"- **Workflow**: {workflow if workflow else '(not available)'}",
                f"- **Run URL**: {run_url if run_url else '(not available)'}",
                "",
                "## Notes",
